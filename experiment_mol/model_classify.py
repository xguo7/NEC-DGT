...
def mse(label,real):
            ...
            for i in range(label.shape[0]):
               score=mean_squared_error(label[i],real[i])
               mse+=score
...
def r2(label,real):
            ...
            for i in range(label.shape[0]):
                for j in range(label.shape[2]):
                        score=r2_score(label[i,:,j],real[i,:,j])
                        r2+=score
            return r2/(label.shape[0]*label.shape[2])

def pear(label,real):
            p=[]
            for i in range(label.shape[0]):
                for j in range(label.shape[2]):                   
                   score=pearsonr(label[i,:,j],real[i,:,j])[0]   
...
class graph2graph(object):
    ...
    def train(self, args):
        ...
        #read data
        node_data_train,node_data_test,Ra_data_train,Ra_data_test,Ra_label_train,Ra_label_test,Rr_data,Rs_data,n_node_list=read_data()
        ...
        for i in range(max_epoches):
          ...
          for j in range(int(len(node_data_train)/self.mini_batch_num)):
            ...
            batch_O_target=node_data_train[j*self.mini_batch_num:(j+1)*self.mini_batch_num];
          ...
          mse_n=mse(node_data_train,np.array(O_t).reshape(node_data_train.shape[0],node_data_train.shape[1],node_data_train.shape[2]))
    ...
    def test(self,args):        
        ...
        for j in range(int(len(node_data_test)/self.mini_batch_num)):
          ...
          batch_O_target=node_data_test[j*self.mini_batch_num:(j+1)*self.mini_batch_num];
        ...
        np.save('O_y'+str(self.No)+'.npy',node_data_test.reshape(len(node_data_test),self.Ds,self.No))
        ...
        print('mse-node: '+str(mse(node_data_test,O_t)))
        #print('mse-edge: '+str(mse(Ra_label_test,Ra_t)))
        print('r2-node: '+str(r2(node_data_test,O_t)))
        #print('r2-edge: '+str(r2(Ra_label_test,Ra_t)))
        print('p-node: '+str(pear(node_data_test,O_t)))
        #print('p-edge: '+str(pear(Ra_label_test,Ra_t)))
        print('sp-node: '+str(spear(node_data_test,O_t)))
